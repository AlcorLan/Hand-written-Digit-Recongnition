...
from utils.visualization import plot_feature_importance, plot_dimensionality_reduction

# Configure logging at the beginning of the script
# Log file will now be saved in the Results/RFC directory
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

style.use('ggplot')

def run_rfc_model(project_root_path=_project_root_path, log_file_path=None):
    """
    ...
    # Define the output directory for RFC results
    rfc_results_dir = os.path.join(project_root_path, 'Results', 'RFC')
    os.makedirs(rfc_results_dir, exist_ok=True)

    # Set up logging to the new log file path
    if log_file_path is None:
        log_file_path = os.path.join(rfc_results_dir, 'summary.log')
    ...
    try:
        ...
        logging.info('\nRandom Forest Classifier Parameters: n_estimators=100, n_jobs=10') # Log model parameters
        print('\nPickling the Classifier for Future Use...')
        clf = RandomForestClassifier(n_estimators=100, n_jobs=10)
        clf.fit(X_train, y_train)

        # Save and Load the model
        pickle_path = os.path.join(rfc_results_dir, 'MNIST_RFC.pickle')
        ...
        confusion_matrix_save_path = os.path.join(rfc_results_dir, 'RFC_Confusion_Matrices.png')
        prediction_examples_save_path = os.path.join(rfc_results_dir, 'RFC_Prediction_Examples.png')
        feature_importance_save_path = os.path.join(rfc_results_dir, 'RFC_Feature_Importance.png')

        plot_confusion_matrices(y_test, y_pred, test_labels, test_labels_pred, title_prefix="RFC ", save_path=confusion_matrix_save_path)
        print_classification_report(y_test, y_pred, title="RFC Validation Data ")
        print_classification_report(test_labels, test_labels_pred, title="RFC Test Data ")

        display_prediction_examples(test_img, test_labels, test_labels_pred, save_path=prediction_examples_save_path)

        # Visualize feature importance
        print('\nVisualizing Feature Importance...')
        # Feature names are simply pixel indices for image data
